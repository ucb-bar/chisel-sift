...
class SymmetricFIR(delay: Int, line: Int, n_tap: Int, dwidth : Int = 8,
  ...
  val tap_delay = mid_tap * delay

  // Total delay from in to out
  val prime_delay =  tap_delay + mul_delay + sum_delay
  ...
  val in_counter = Module(new Counter(UInt(line-1)))
  in_counter.io.en := io.in.valid

  // Counter for line to disable taps when at edge
  val tap_counter = Module(new Counter(UInt(line-1)))
  ...
  val prime = (tap_counter.io.count === UInt(0) & 
    in_counter.io.count < UInt(tap_delay-1))
  ...
  advance_taps := io.in.valid || flush
  ...
  for (i <- 0 until mid_tap) {
    mul_out_d(i) := ShiftRegister(mul_out(i), mul_delay)
  }
  
  val mul_valid = ShiftRegister(taps_valid, mul_delay)
  ...
  for (tap_idx <- 0 until mid_tap - 1) {
    // Low-side muxes
    terms(tap_idx) := Mux(
      ShiftRegister(tap_counter.io.count > UInt(tap_idx), mul_delay), 
      mul_out_d(tap_idx), UInt(0))

    // High-side muxes and delays
    terms(n_tap-tap_idx-1) := Mux(
      ShiftRegister(tap_counter.io.count < UInt(line-tap_idx-1), mul_delay),
      ShiftRegister(
        mul_out_d(tap_idx),
  ...
  val sum_valid = ShiftRegister(mul_valid, sum_delay)
